...
void            *td_computeCloudFall(void *p_mega)
{
    ...
    *((uint *)pThread->local) = 0;
    while (dParticle < tTo && mega->thread.global == 1)
    {
        ...
        {
            model = data->model[i];
            if (model.active) // If model is active
            {
...
void            *td_displayLoading(void *p_mega)
{
    ...
    if (!(zCloud = mega->stcl->cloud_bufsz))
        pthread_exit(NULL);
    if (!(loadingBar = new_surface32(mega->viewport->w, 10 + 64)))
        pthread_exit(NULL);
    clean_surface(loadingBar);
    charset_print(mega->screen, mega->charset, 0, mega->viewport->h - 26, "Loading... [Press 's' to abort]");
    tSum = 0;
    ...
    while (tSum < tTo - 16)
    {
        ...
        {
            tSum = count;
            fPercent = (float)tSum * fUnit;
            loading_bar(loadingBar, fPercent);
            blit_and_flip_at(loadingBar, mega->screen, 0, mega->viewport->h - (10 + 64)); // Render !
        }
        //
        SDL_PumpEvents();
...
int             user_computeCloud(t_mega *mega, int flg)
{
    ...
    /* ********* INIT CAMERA ********** */
    ///return (clean_pointer(screen, viewport, picture, charset, camera, &stcl, 1));
